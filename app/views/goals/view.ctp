...
<fieldset>
	...
	<tr <?php if ($i++ % 2 == 0) {echo $class;}?> >
		...
		<td><?php echo $this->Html->link($goal['Aspect']['name'], array('controller' => 'aspects', 'action' => 'view', $goal['Aspect']['id'])); ?>&nbsp;</td>
	</tr>
	<tr <?php if ($i++ % 2 == 0) {echo $class;}?> >
		<td><?php __('Kompetenzbereich'); ?>&nbsp;</td>
		<td><?php echo $this->Html->link($goal['Competency']['name'], array('controller' => 'competencies', 'action' => 'view', $goal['Competency']['id'])); ?>&nbsp;</td>
	</tr>
	<tr <?php if ($i++ % 2 == 0) {echo $class;}?> >
		<td><?php __('Beschreibung'); ?>&nbsp;</td>
		<td><?php echo $goal['Aspect']['description']; ?>&nbsp;</td>
	</tr>
</table>
	<br>
	<div class="actions">
		<?php echo $this->Html->link(__('Lernziel bearbeiten', true), array('action' => 'edit', $goal['Goal']['id'])); ?> 
		<?php echo $this->Html->link(__('Lernziel Löschen', true), array('action' => 'delete', $goal['Goal']['id']), null, sprintf(__('Soll der Handlungsaspekt %s wirklich gelöscht werden?', true), $goal['Goal']['name'])); ?> 
	</div>
</fieldset>

<fieldset>
	<legend><?php __("Testaufgaben mit diesem Lernziel"); ?></legend>
	<table cellpadding="0" cellspacing="0">
	<tr>
	...
	<?php
	$i = 0;
	foreach ($goal['Exercise'] as $exercise):
		$class = null;
		$altrow = 0;
		$i++;
		if ($i % 2 == 0) {
