...
<fieldset>
	<legend><?php __($competency['Competency']['name']); ?></legend>
	...
	<?php 
		if($admin)
		{
			echo $this->Html->link(__('Kompetenzbereich bearbeiten', true), array('action' => 'edit', $competency['Competency']['id']));
			echo $this->Html->link(__('Kompetenzbereich Löschen', true), array('action' => 'delete', $competency['Competency']['id']), null, sprintf(__('Soll der Handlungsaspekt %s wirklich gelöscht werden?', true), $competency['Competency']['name']));
		} 
	?> 
	</div>
</fieldset>
<fieldset>
	<legend><?php __("Testaufgaben mit diesem Kompetenzbereich"); ?></legend>
	<table cellpadding="0" cellspacing="0">
	<tr>
	...
	<?php
	$i = 0;
	foreach ($competency['Exercise'] as $exercise):
		$class = null;
		$altrow = 0;
		$i++;
		if ($i % 2 == 0) {
...
<fieldset>
	<legend><?php __("Lernziele mit diesem Kompetenzbereich"); ?></legend>
	<table cellpadding="0" cellspacing="0">
	<tr>
	...
	<?php
	$i = 0;
	foreach ($competency['Goal'] as $goal):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}?>
	...
	<tr<?php echo $class;?>>
		...
		<td>
			<?php echo $this->Html->link($Aspect[$goal['aspect_id']], array('controller' => 'aspects', 'action' => 'view', $goal['aspect_id'])); ?>
		</td>
		...
		<td class="actions">
			...
			if($admin)
			{
				$this->Html->image('layout/Edit-Document.png', array('alt'=> __('Bearbeiten',true), 'url' => array('controller' => 'goals', 'action' => 'edit', $goal['id']))); 
				$this->Html->image('layout/File-Delete.png', array('alt'=>'Löschen', 'url' => array('controller' => 'exercises','action' => 'delete', $goal['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $goal['name']))); 				
			}?>
